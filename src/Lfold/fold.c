...
*/
PUBLIC int logML =
    0; /* if nonzero use logarithmic ML energy in energy_of_struct */
...
*/
PRIVATE int *indx =
    NULL; /* index for moving in the triangle matrices c[] and fMl[]*/
...
PRIVATE int *f53 =
    NULL; /* energy of 5' end with 3' nucleotide not available for mismatches */
PRIVATE int *fML = NULL;    /* multi-loop auxiliary energy array */
PRIVATE int *fM1 = NULL;    /* second ML array, only for subopt */
PRIVATE int *fM2 = NULL;    /* fM2 = multiloop region with exactly two stems,
                               extending to 3' end        */
...
PRIVATE int *DMLi_a = NULL; /* DMLi_a[j] holds min energy for at least two
                               multiloop stems in [i,j], where j is available
                               for dangling onto a surrounding stem */
PRIVATE int *DMLi_o = NULL; /* DMLi_o[j] holds min energy for at least two
                               multiloop stems in [i,j], where j is unavailable
                               for dangling onto a surrounding stem */
...
PRIVATE sect
    sector[MAXSECTORS];     /* stack of partial structures for backtracking */
...
#pragma omp threadprivate(indx, c, cc, cc1, f5, f53, fML, fM1, fM2, Fmi, DMLi, \
                          DMLi1, DMLi2, DMLi_a, DMLi_o, DMLi1_a, DMLi1_o,      \
                          DMLi2_a, DMLi2_o, Fc, FcH, FcI, FcM, sector, ptype,  \
                          S, S1, P, init_length, BP, pair_table, base_pair2,   \
                          circular, struct_constrained, ggg, with_gquad)
...
int LoopEnergy(int n1, int n2, int type, int type_2, int si1, int sj1, int sp1,
               int sq1);
...
PRIVATE void get_arrays(unsigned int size) {
  ...
  if (circular)
    fM2 = (int *)space(sizeof(int) * (size + 2));
}
...
PUBLIC void free_arrays(void) {
  ...
  indx = c = fML = f5 = f53 = cc = cc1 = fM1 = fM2 = Fmi = DMLi = DMLi1 =
      DMLi2 = ggg = NULL;
...
PUBLIC void export_fold_arrays_par(int **f5_p, int **c_p, int **fML_p,
...
PUBLIC void export_circfold_arrays(int *Fc_p, int *FcH_p, int *FcI_p,
                                   int *FcM_p, int **fM2_p, int **f5_p,
                                   int **c_p, int **fML_p, int **fM1_p,
                                   int **indx_p, char **ptype_p) {
  /* make the DP arrays available to routines such as subopt() */
  *f5_p = f5;
  *c_p = c;
...
PUBLIC void export_circfold_arrays_par(int *Fc_p, int *FcH_p, int *FcI_p,
                                       int *FcM_p, int **fM2_p, int **f5_p,
                                       int **c_p, int **fML_p, int **fM1_p,
                                       int **indx_p, char **ptype_p,
                                       paramT **P_p) {
  export_circfold_arrays(Fc_p, FcH_p, FcI_p, FcM_p, fM2_p, f5_p, c_p, fML_p,
                         fM1_p, indx_p, ptype_p);
  *P_p = P;
}
/*--------------------------------------------------------------------------*/
...
PUBLIC float fold_par(const char *string, char *structure, paramT *parameters,
                      int is_constrained, int is_circular) {
  ...
  if (with_gquad) { /* add a guess of how many G's may be involved in a G
                       quadruplex */
    if (base_pair2)
      free(base_pair2);
  ...
  */
  ...
  for (i = 1; i <= length; i++) {
    if ((BP[i] < 0) && (BP[i] > -4)) {
      bonus_cnt++;
      if ((BP[i] == -3) && (structure[i - 1] == ')'))
        bonus++;
      if ((BP[i] == -2) && (structure[i - 1] == '('))
        bonus++;
      if ((BP[i] == -1) && (structure[i - 1] != '.'))
        bonus++;
    }
    ...
    if (BP[i] > i) {
      ...
      for (l = 1; l <= base_pair2[0].i; l++)
        if (base_pair2[l].i != base_pair2[l].j)
          if ((i == base_pair2[l].i) && (BP[i] == base_pair2[l].j))
            bonus++;
    }
  }
...
PRIVATE int fill_arrays(const char *string) {
  ...
  max_separation =
      (int)((1. - LOCALITY) * (double)(length - 2)); /* not in use */
  ...
  for (i = length - TURN - 1; i >= 1; i--) { /* i,j in [1..length] */
    ...
    for (j = i + TURN + 1; j <= length; j++) {
      ...
      if (type) { /* we have a pair */
        ...
        new_c =
            (no_close) ? FORBIDDEN : E_Hairpin(j - i - 1, type, S1[i + 1],
                                               S1[j - 1], string + i - 1, P);
        ...
        for (p = i + 1; p <= MIN2(j - 2 - TURN, i + MAXLOOP + 1); p++) {
          ...
          for (q = minq; q < j; q++) {
            ...
            energy = E_IntLoop(p - i - 1, j - q - 1, type, type_2, S1[i + 1],
                               S1[j - 1], S1[p - 1], S1[q + 1], P);
        ...
        if (!no_close) {
          ...
          default:
            ...
            decomp =
                MIN2(decomp,
                     DMLi2[j - 1] + E_MLstem(tt, -1, S1[i + 1], P) + P->MLbase);
            decomp = MIN2(decomp, DMLi2[j - 2] +
                                      E_MLstem(tt, S1[j - 1], S1[i + 1], P) +
                                      2 * P->MLbase);
            decomp =
                MIN2(decomp,
                     DMLi1[j - 2] + E_MLstem(tt, S1[j - 1], -1, P) + P->MLbase);
            break;
          }
        ...
        if (dangle_model == 3) {
          ...
          for (k = i + 2 + TURN; k < j - 2 - TURN; k++) {
            ...
            if (type_2)
              decomp =
                  MIN2(decomp, c[indx[k] + i + 1] + P->stack[type][type_2] +
                                   fML[indx[j - 1] + k + 1]);
            type_2 = rtype[ptype[indx[j - 1] + k + 1]];
            if (type_2)
              decomp =
                  MIN2(decomp, c[indx[j - 1] + k + 1] + P->stack[type][type_2] +
                                   fML[indx[k] + i + 1]);
          }
        ...
        if (with_gquad) {
          /* include all cases where a g-quadruplex may be enclosed by base pair
           * (i,j) */
          if (!no_close) {
            tt = rtype[type];
      ...
      if (type) {
        ...
        case 2:
          new_fML +=
              E_MLstem(type, (i == 1) ? S1[length] : S1[i - 1], S1[j + 1], P);
          break;
        default:
          new_fML += E_MLstem(type, -1, -1, P);
          break;
        }
      }
      ...
      */
      ...
      default:
        ...
        if (tt)
          new_fML =
              MIN2(new_fML, c[ij + 1] + E_MLstem(tt, mm5, -1, P) + P->MLbase);
        tt = ptype[indx[j - 1] + i];
        if (tt)
          new_fML = MIN2(new_fML, c[indx[j - 1] + i] +
                                      E_MLstem(tt, -1, mm3, P) + P->MLbase);
        tt = ptype[indx[j - 1] + i + 1];
        if (tt)
          new_fML =
              MIN2(new_fML, c[indx[j - 1] + i + 1] + E_MLstem(tt, mm5, mm3, P) +
                                2 * P->MLbase);
        break;
      }
      ...
      if (dangle_model == 3) {
        ...
        for (decomp = INF, k = i + 1 + TURN; k <= j - 2 - TURN; k++) {
          ...
          type_2 = rtype[type_2];
          if (type && type_2)
            decomp = MIN2(decomp, c[indx[k] + i] + c[indx[j] + k + 1] +
                                      P->stack[type][type_2]);
        }
  ...
  case 2:
    for (j = TURN + 2; j < length; j++) {
      ...
      for (i = j - TURN - 1; i > 1; i--) {
        ...
        en = c[indx[j] + i];
        f5[j] = MIN2(f5[j],
                     f5[i - 1] + en + E_ExtLoop(type, S1[i - 1], S1[j + 1], P));
      }
    ...
    for (i = length - TURN - 1; i > 1; i--) {
      ...
      en = c[indx[length] + i];
      f5[length] =
          MIN2(f5[length], f5[i - 1] + en + E_ExtLoop(type, S1[i - 1], -1, P));
    }
    ...
    for (j = TURN + 2; j <= length; j++) {
      ...
      for (i = j - TURN - 1; i > 1; i--) {
        ...
        if (type) {
          ...
          f5[j] = MIN2(f5[j], f5[i - 1] + en + E_ExtLoop(type, -1, -1, P));
          f5[j] =
              MIN2(f5[j], f5[i - 2] + en + E_ExtLoop(type, S1[i - 1], -1, P));
        }
        ...
        if (type) {
          ...
          f5[j] = MIN2(f5[j],
                       f5[i - 2] + en + E_ExtLoop(type, S1[i - 1], S1[j], P));
        }
      }
      ...
      type = ptype[indx[j] + 1];
      if (type)
        f5[j] = MIN2(f5[j], c[indx[j] + 1] + E_ExtLoop(type, -1, -1, P));
      type = ptype[indx[j - 1] + 1];
      if (type)
        f5[j] = MIN2(f5[j], c[indx[j - 1] + 1] + E_ExtLoop(type, -1, S1[j], P));
    }
  }
  return f5[length];
}
...
PRIVATE void backtrack(const char *string, int s) {
    ...
    if (ml == 0) { /* backtrack in f5 */
      ...
      case 0: /* j is paired. Find pairing partner */
        for (k = j - TURN - 1, traced = 0; k >= 1; k--) {
          ...
          if (type)
            if (fij ==
                E_ExtLoop(type, -1, -1, P) + c[indx[j] + k] + f5[k - 1]) {
              traced = j;
              jj = k - 1;
              break;
            }
      ...
      case 2:
        ...
        for (k = j - TURN - 1, traced = 0; k >= 1; k--) {
          ...
          if (type)
            if (fij ==
                E_ExtLoop(type, (k > 1) ? S1[k - 1] : -1, mm3, P) +
                    c[indx[j] + k] + f5[k - 1]) {
              traced = j;
              jj = k - 1;
              break;
      ...
      default:
        ...
        tt = ptype[indx[j - 1] + i];
        if (fij ==
            c[indx[j - 1] + i] + E_MLstem(tt, -1, S1[j], P) + P->MLbase) {
          base_pair2[++b].i = i;
          base_pair2[b].j = --j;
          goto repeat1;
        }
        tt = ptype[indx[j - 1] + i + 1];
        if (fij ==
            c[indx[j - 1] + i + 1] + E_MLstem(tt, S1[i], S1[j], P) +
                2 * P->MLbase) {
          base_pair2[++b].i = ++i;
          base_pair2[b].j = --j;
          goto repeat1;
  ...
  repeat1:
    ...
    if (no_close) {
      if (cij == FORBIDDEN)
        continue;
               ...
               E_Hairpin(j - i - 1, type, S1[i + 1], S1[j - 1], string + i - 1,
                         P) +
                   bonus)
      continue;
    ...
    if (with_gquad) {
      ...
      */
      if (backtrack_GQuad_IntLoop(cij - bonus, i, j, type, S, ggg, indx, &p, &q,
                                  P)) {
        i = p;
        j = q;
        goto repeat_gquad;
    ...
    switch (dangle_model) {
    case 0:
      en = cij - E_MLstem(tt, -1, -1, P) - P->MLclosing - bonus;
      for (k = i + 2 + TURN; k < j - 2 - TURN; k++) {
    ...
    default:
      for (k = i + 2 + TURN; k < j - 2 - TURN; k++) {
        ...
        if (en ==
            fML[indx[k] + i + 1] + fML[indx[j - 1] + k + 1] +
                E_MLstem(tt, -1, -1, P)) {
          break;
        } else if (en ==
                   fML[indx[k] + i + 2] + fML[indx[j - 1] + k + 1] +
                       E_MLstem(tt, -1, S1[i + 1], P) + P->MLbase) {
          i1 = i + 2;
          break;
        } else if (en ==
                   fML[indx[k] + i + 1] + fML[indx[j - 2] + k + 1] +
                       E_MLstem(tt, S1[j - 1], -1, P) + P->MLbase) {
          j1 = j - 2;
          break;
        } else if (en ==
                   fML[indx[k] + i + 2] + fML[indx[j - 2] + k + 1] +
    ...
    continue; /* this is a workarround to not accidentally proceed in the
                 following block */
...
PUBLIC void update_fold_params(void) { update_fold_params_par(NULL); }
...
/*---------------------------------------------------------------------------*/
PUBLIC float energy_of_structure(const char *string, const char *structure,
                                 int verbosity_level) {
  return energy_of_struct_par(string, structure, NULL, verbosity_level);
}

PUBLIC float energy_of_struct_par(const char *string, const char *structure,
                                  paramT *parameters, int verbosity_level) {
  int energy;
  short *ss, *ss1;
...
/*  returns a correction term that may be added to the energy retrieved
    ...
    recursive variant
*/
PRIVATE int en_corr_of_loop_gquad(int i, int j, const char *string,
                                  const char *structure, short *pt,
                                  int *loop_idx, const short *s1) {
  ...
  while ((pos = parse_gquad(structure + q - 1, &L, l)) > 0) {
    ...
    if (loop_idx[p] == 0) {
      ...
      energy +=
          E_MLstem(0, -1, -1, P); /*  do not forget to remove this energy if
      ...
      for (r = p - 1; !pt[r] && (r >= i); r--)
        ;
      if (r < i)
        nrerror("this should not happen");
      ...
      while (u < s) {
        if (structure[u - 1] == '.')
          ...
          elem_i = u;
          elem_j = pt[u];
          energy += en_corr_of_loop_gquad(u, pt[u], string, structure, pt,
                                          loop_idx, s1);
          u = pt[u] + 1;
        }
      }
      if (u != s)
        nrerror("what the hell");
        ...
        case 0: /* if(num_g == 1)
          ...
          energy -= E_Hairpin(s - r - 1, type, s1[r + 1], s1[s - 1],
                              string + r - 1, P);
          break;
        /* g-quad was misinterpreted as interior loop closed by (r,s) with
         * enclosed pair (elem_i, elem_j) */
        case 1:
          type = pair[s1[r]][s1[s]];
          ...
          energy -= E_IntLoop(elem_i - r - 1, s - elem_j - 1, type,
                              rtype[type2], s1[r + 1], s1[s - 1],
                              s1[elem_i - 1], s1[elem_j + 1], P);
          break;
...
PUBLIC float energy_of_gquad_struct_par(const char *string,
                                        const char *structure,
                                        paramT *parameters,
                                        int verbosity_level) {
  ...
  */
  ...
  gge = en_corr_of_loop_gquad(1, S[0], string, structure, pair_table, loop_idx,
                              S1);
  energy += gge;
...
PUBLIC int energy_of_structure_pt(const char *string, short *ptable, short *s,
                                  short *s1, int verbosity_level) {
  return energy_of_struct_pt_par(string, ptable, s, s1, NULL, verbosity_level);
}

PUBLIC int energy_of_struct_pt_par(const char *string, short *ptable, short *s,
                                   short *s1, paramT *parameters,
                                   int verbosity_level) {
  ...
  energy = backtrack_type == 'M' ? energy_of_ml_pt(0, ptable)
                                 : energy_of_extLoop_pt(0, ptable);
...
PUBLIC float energy_of_circ_structure(const char *string, const char *structure,
                                      int verbosity_level) {
...
PUBLIC float energy_of_circ_struct_par(const char *string,
  ...
  if (degree == 1) {
    ...
    en0 = E_IntLoop(u1, u2, type, type_2, S1[j + 1], si1, S1[p - 1], sq1, P);
  } else { /* degree > 2 */
...
PRIVATE int stack_energy(int i, const char *string, int verbosity_level) {
  ...
  while (p < q) { /* process all stacks and interior loops */
    ...
    if (SAME_STRAND(i, p) && SAME_STRAND(q, j))
      ee = E_IntLoop(p - i - 1, j - q - 1, type, type_2, S1[i + 1], S1[j - 1],
                     S1[p - 1], S1[q + 1], P);
    else
      ee = energy_of_extLoop_pt(cut_in_loop(i), pair_table);
    if (verbosity_level > 0)
      printf("Interior loop (%3d,%3d) %c%c; (%3d,%3d) %c%c: %5d\n", i, j,
             string[i - 1], string[j - 1], p, q, string[p - 1], string[q - 1],
             ee);
    energy += ee;
    i = p;
    j = q;
  ...
  }
  {
    int ii;
    ii = cut_in_loop(i);
    ee = (ii == 0) ? energy_of_ml_pt(i, pair_table)
                   : energy_of_extLoop_pt(ii, pair_table);
  }
  energy += ee;
  if (verbosity_level > 0)
...
PRIVATE int energy_of_extLoop_pt(int i, short *pair_table) {
  ...
  int E3_available; /* energy of 5' part where 5' mismatch is available for
                       current stem */
  int E3_occupied;  /* energy of 5' part where 5' mismatch is unavailable for
                       current stem */
  ...
  while (p < length) {
    ...
    case 2:
      mm5 = ((SAME_STRAND(p - 1, p)) && (p > 1)) ? S1[p - 1] : -1;
    ...
    default: {
      ...
      mm5 = ((SAME_STRAND(p - 1, p)) && (p > 1) && !pair_table[p - 1])
                ? S1[p - 1]
                : -1;
      mm3 = ((SAME_STRAND(q, q + 1)) && (q < length) && !pair_table[q + 1])
                ? S1[q + 1]
                : -1;
      tmp = MIN2(E3_occupied + E_ExtLoop(tt, -1, mm3, P),
                 E3_available + E_ExtLoop(tt, mm5, mm3, P));
      E3_available = MIN2(E3_occupied + E_ExtLoop(tt, -1, -1, P),
...
PRIVATE int energy_of_ml_pt(int i, short *pt) {
  ...
  int i1, j, p, q, q_prev, q_prev2, u, x, type, count, mm5, mm3, tt, ld5,
      new_cx, dang5, dang3, dang;
  int mlintern[NBPAIRS + 1];

  /* helper variables for dangles == 1|5 case */
  int E_mm5_available; /* energy of 5' part where 5' mismatch of current stem is
                          available */
  int E_mm5_occupied;  /* energy of 5' part where 5' mismatch of current stem is
                          unavailable */
  int E2_mm5_available; /* energy of 5' part where 5' mismatch of current stem
                           is available with possible 3' dangle for enclosing
                           pair (i,j) */
  int E2_mm5_occupied; /* energy of 5' part where 5' mismatch of current stem is
                          unavailable with possible 3' dangle for enclosing pair
                          (i,j) */
  int dangle_model = P->model_details.dangles;
    ...
    for (count = 0; count < 2; count++) { /* do it twice */
      ...
      if (i == 0) {
        ...
        if (SAME_STRAND(j - 1, j)) {
          ...
          if ((p = (unsigned int)pair_table[j - 2]) &&
              SAME_STRAND(j - 2, j - 1))
      ...
      do { /* walk around the multi-loop */
        ...
        while (p <= (unsigned int)pair_table[0] && pair_table[p] == 0)
          p++;
      ...
      while (pair_table[p] == 0)
        p++;
      if (i == (unsigned int)pair_table[p])
        break;
  ...
  default:
    ...
    while (p < j) {
       ...
       * E_mm5_occupied,E_mm5_available,E2_mm5_occupied,E2_mm5_available); */
      /* seek to the next stem */
      p = q + 1;
      q_prev = q_prev2 = q;
      while (p <= j && !pair_table[p])
...
PUBLIC int loop_energy(short *ptable, short *s, short *s1, int i) {
  ...
  j = pair_table[i];
  if (j < i)
    nrerror("i is unpaired in loop_energy()");
  type = pair[S[i]][S[j]];
  if (type == 0) {
    type = 7;
    if (eos_debug >= 0)
  ...
  if (p > q) { /* Hairpin */
    ...
    int ii;
    ii = cut_in_loop(i);
    energy = (ii == 0) ? energy_of_ml_pt(i, pair_table)
                       : energy_of_extLoop_pt(ii, pair_table);
...
PUBLIC float energy_of_move(const char *string, const char *structure, int m1,
                            int m2) {
  int energy;
  short *ss, *ss1;
...
PUBLIC int energy_of_move_pt(short *pt, short *s, short *s1, int m1, int m2) {
  ...
  if (!SAME_STRAND(k, l)) {
    ...
    else if (c == 0) /* Must have been a delete move */
      return (en_post - en_pre + P->DuplexInit);
  }
    ...
    if (size == 3) {
      ...
      if (type > 2)              /* neither CG nor GC */
        energy += P->TerminalAU; /* penalty for closing AU GU pair IVOO??
                                    sind dass jetzt beaunuesse oder mahlnuesse
                                    (vorzeichen?)*/
      return energy;
    }
  }
...
PUBLIC int LoopEnergy(int n1, int n2, int type, int type_2, int si1, int sj1,
                      int sp1, int sq1) {
  /* compute energy of degree 2 loop (stack bulge or interior) */
  int nl, ns, energy;
  ...
  if (ns == 0) { /* bulge */
    ...
    if (ns == 1) {
      ...
      if (nl == 2) { /* 2x1 loop */
        ...
        energy +=
            P->mismatch1nI[type][si1][sj1] + P->mismatch1nI[type_2][sq1][sp1];
        return energy;
      }
    } else if (ns == 2) {
      if (nl == 2) { /* 2x2 loop */
        ...
        energy +=
            P->mismatch23I[type][si1][sj1] + P->mismatch23I[type_2][sq1][sp1];
        return energy;
      }
    }
...
PRIVATE int ML_Energy(int i, int is_extloop) {
  ...
  */
  if (is_extloop) {
    ...
    while (p <= (int)pair_table[0] && pair_table[p] == 0)
      p++;
    /* get position of pairing partner */
    ...
    if (p < (int)pair_table[0])
      do {
        ...
        int mm3 = ((SAME_STRAND(q, q + 1)) && (q < (unsigned int)pair_table[0]))
                      ? S1[q + 1]
                      : -1;
        ...
        case 1: {
          ...
          if (!u3) {
            mm3 = -1;
            E_mm5_occupied = MIN2(E_mm5_occupied + E_ExtLoop(tt, -1, -1, P),
                                  E_mm5_available + E_ExtLoop(tt, mm5, -1, P));
            E_mm5_available = E_mm5_occupied;
          } else {
            E_mm5_occupied = MIN2(E_mm5_occupied + E_ExtLoop(tt, -1, mm3, P),
                                  E_mm5_available + E_ExtLoop(tt, mm5, mm3, P));
            E_mm5_available = MIN2(E_mm5_occupied + E_ExtLoop(tt, -1, -1, P),
                                   E_mm5_available + E_ExtLoop(tt, mm5, -1, P));
          }
        } break;
    ...
    for (count = 0; count < 2; count++) { /* do it twice */
      ...
      do { /* walk around the multi-loop */
        ...
        if (dangle_model) {
          ...
          if ((SAME_STRAND(i1, i1 + 1)) && (i1 < (unsigned int)S[0]))
            dang3 =
                P->dangle3[type][S1[i1 + 1]]; /* 3'dangle of previous pair */
          ...
          case 0: /* adjacent helices */
            if (dangle_model == 2)
              ...
              if (SAME_STRAND(i1, p)) {
                ...
                /* subtract 5'dangle and TerminalAU penalty */
                new_cx +=
                    -ld5 - mlintern[tt] - mlintern[type] + 2 * mlintern[1];
              }
              ld5 = 0;
              energy = MIN2(energy, cx_energy);
            }
...
PUBLIC int energy_of_struct_pt(const char *string, short *ptable, short *s,
                               short *s1) {
