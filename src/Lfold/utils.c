...
PUBLIC unsigned int get_multi_input_line(char **string, unsigned int option) {
    ...
    */
    ...
    case '@': /* user abort */
      if (state)
        ...
        free(line);
      return (state == 2) ? VRNA_INPUT_CONSTRAINT : (state == 1)
                                                        ? VRNA_INPUT_SEQUENCE
                                                        : VRNA_INPUT_QUIT;
    ...
    case '\0': /* empty line */
      if (option & VRNA_INPUT_NOSKIP_BLANK_LINES) {
        ...
        return (state == 2)
                   ...
                   : (state == 1) ? VRNA_INPUT_SEQUENCE : VRNA_INPUT_BLANK_LINE;
      }
      break;
    ...
    case '>': /* fasta header */
      ...
      return (state == 2)
                 ? VRNA_INPUT_CONSTRAINT
                 : (state == 1) ? VRNA_INPUT_SEQUENCE : VRNA_INPUT_FASTA_HEADER;

    case 'x':
    case 'e':
    case 'l':
    case '&': /* seems to be a constraint or line starting with second sequence
                 for dimer calculations */
      i = 1;
      /* lets see if this assumption holds for the complete line */
      while ((line[i] == 'x') || (line[i] == 'e') || (line[i] == 'l'))
        i++;
      /* lines solely consisting of 'x's, 'e's or 'l's will be considered as
       * structure constraint */

      if (((line[i] > 64) && (line[i] < 91))     /* A-Z */
          || ((line[i] > 96) && (line[i] < 123)) /* a-z */
          ) {
        if (option & VRNA_INPUT_FASTA_HEADER) {
          /* are we in structure mode? Then we remember this line for the next
           * round */
          if (state == 2) {
            inbuf = line;
            ...
            *string =
                (char *)xrealloc(*string, sizeof(char) * (str_length + l + 1));
  ...
  return (state == 2) ? VRNA_INPUT_CONSTRAINT
                      : (state == 1) ? VRNA_INPUT_SEQUENCE : VRNA_INPUT_ERROR;
}
...
PUBLIC unsigned int read_record(char **header, char **sequence, char ***rest,
  ...
  *rest = (char **)space(sizeof(char *));
  ...
  while (input_type &
         (VRNA_INPUT_MISC | VRNA_INPUT_CONSTRAINT | VRNA_INPUT_BLANK_LINE)) {
    free(input_string);
    input_string = NULL;
    input_type = get_multi_input_line(&input_string, options);
    if (input_type & (VRNA_INPUT_QUIT | VRNA_INPUT_ERROR))
  ...
  if (input_type & VRNA_INPUT_FASTA_HEADER) {
    return_type |=
        VRNA_INPUT_FASTA_HEADER; /* remember that we've read a fasta header */
    *header = input_string;
    input_string = NULL;
    /* get next data-block with fasta support if not explicitely forbidden by
     * VRNA_INPUT_NO_SPAN */
    input_type = get_multi_input_line(
        &input_string,
        ((options & VRNA_INPUT_NO_SPAN) ? 0 : VRNA_INPUT_FASTA_HEADER) |
            options);
    if (input_type & (VRNA_INPUT_QUIT | VRNA_INPUT_ERROR))
  ...
  if (input_type & VRNA_INPUT_SEQUENCE) {
    return_type |=
        VRNA_INPUT_SEQUENCE; /* remember that we've read a sequence */
    *sequence = input_string;
    input_string = NULL;
  } else
    nrerror("sequence input missing");

  /* read the rest until we find user abort, EOF, new sequence or new fasta
   * header */
  if (!(options & VRNA_INPUT_NO_REST)) {
    options |= VRNA_INPUT_NOSKIP_COMMENTS; /* allow commetns to appear in rest
                                              output */
    tmp_type = VRNA_INPUT_QUIT | VRNA_INPUT_ERROR | VRNA_INPUT_SEQUENCE |
               VRNA_INPUT_FASTA_HEADER;
    if (options & VRNA_INPUT_NOSKIP_BLANK_LINES)
      tmp_type |= VRNA_INPUT_BLANK_LINE;
    while (!((input_type = get_multi_input_line(&input_string, options)) &
             tmp_type)) {
  ...
  while (i < l) {
    ...
    for (p = pi = 0; pi < 5; pi++) {
      ...
      case ')':
        ...
        break;
      default:
        nrerror("pack_structure: illegal charcter in structure");
      }
      if (i < l)
        i++;
    }
...
PUBLIC void print_tty_constraint_full(void) {
  print_tty_constraint(VRNA_CONSTRAINT_PIPE | VRNA_CONSTRAINT_DOT |
                       VRNA_CONSTRAINT_X | VRNA_CONSTRAINT_ANG_BRACK |
                       VRNA_CONSTRAINT_RND_BRACK);
}

PUBLIC void print_tty_constraint(unsigned int option) {
  if (!(option & VRNA_CONSTRAINT_NO_HEADER))
    printf("Input structure constraints using the following notation:\n");
  if (option & VRNA_CONSTRAINT_PIPE)
    printf("| : paired with another base\n");
  if (option & VRNA_CONSTRAINT_DOT)
    printf(". : no constraint at all\n");
  if (option & VRNA_CONSTRAINT_X)
    printf("x : base must not pair\n");
  if (option & VRNA_CONSTRAINT_ANG_BRACK)
    printf("< : base i is paired with a base j<i\n> : base i is paired with a "
           "base j>i\n");
  if (option & VRNA_CONSTRAINT_RND_BRACK)
...
PUBLIC void getConstraint(char **cstruc, const char **lines,
                          unsigned int option) {
  int r, i, l, cl, stop;
  char *c, *ptr;
  if (lines) {
    if (option & VRNA_CONSTRAINT_ALL)
      option |= VRNA_CONSTRAINT_PIPE | VRNA_CONSTRAINT_ANG_BRACK |
                VRNA_CONSTRAINT_RND_BRACK | VRNA_CONSTRAINT_X |
                VRNA_CONSTRAINT_G;
    ...
    for (r = i = stop = 0; lines[i]; i++) {
      ...
      if ((*c == '#') || (*c == '%') || (*c == ';') || (*c == '/') ||
          (*c == '*' || (*c == '\0'))) {
        /* skip leading comments only, i.e. do not allow comments inside the
         * constraint */
        if (!r)
          continue;
        else
      ...
      for (ptr = c; *c; c++) {
        ...
        case '.':
          break;
        case '&':
          break; /* ignore concatenation char */
        default:
          warn_user("unrecognized character in constraint structure");
          break;
        }
      }
      ...
      *cstruc = (char *)xrealloc(*cstruc, r * sizeof(char));
      ...
      if (!(option & VRNA_CONSTRAINT_MULTILINE) || (cl != l))
        break;
    }
  }
}
...
PUBLIC char *extract_record_rest_structure(const char **lines,
  ...
  if (lines) {
    for (r = i = stop = 0; lines[i]; i++) {
      ...
      if (!(option & VRNA_OPTION_MULTILINE))
        break;
    }
  }
  return structure;
...
PUBLIC void constrain_ptypes(const char *constraint, unsigned int length,
                             char *ptype, int *BP, int min_loop_size,
  ...
  if (!idx_type) { /* index allows access in energy matrices at pos (i,j) via
                      index[j]+i */
...
/* get a matrix containing the number of basepairs of a reference structure for
* each interval [i,j] with i<j
*  access it via iindx!!!
*/
PUBLIC unsigned int *make_referenceBP_array(short *reference_pt,
                                            unsigned int turn) {
  ...
  array = (unsigned int *)space(
      sizeof(unsigned int) * size); /* matrix containing number of basepairs of
  ...
  for (i = length - turn - 1; i >= 1; i--)
    for (j = i + turn + 1; j <= length; j++) {
      ...
      ij = iindx[i] - j;
      bps = array[ij + 1];
      if ((i <= (unsigned int)reference_pt[j]) &&
          ((unsigned int)reference_pt[j] < j))
        bps++;
      array[ij] = bps;
    }
  free(iindx);
...
PUBLIC unsigned int *compute_BPdifferences(short *pt1, short *pt2,
                                           unsigned int turn) {
  unsigned int *array;
