...
/**
 *  \file utils.h
 *  \brief Various utility- and helper-functions used throughout the Vienna RNA
 * package
 */
...
/**
 *  Output flag of \ref get_input_line():  "the user requested quitting the
 * program"
 */
 ...
 *  if used as input option this tells get_input_line() that the data to be read
 *should comply
...
/**
 *  Input switch for read_record():  "never allow data to span more than one
 * line"
 */
...
/**
 *  angle brackets '<', '>' switch for structure constraint (paired
 * downstream/upstream)
 */
...
/**
 * Tell a function that an input is assumed to span several lines if used as
 *input-option
 * A function might also be returning this state telling that it has read data
 *from
 * multiple lines.
 *
 ...
 *  \return         A random string of length 'l' containing characters from the
 *symbolset
 */
/*@only@*/ /*@notnull@*/
...
/**
 *  \brief Calculate hamming distance between two sequences up to a specified
 *length
 *
 *  This function is similar to hamming() but instead of comparing both
 *sequences
 *  up to their actual length only the first 'n' characters are taken into
 *account
 *  \param s1   The first sequence
 *  \param s2   The second sequence
 *  \return     The hamming distance between s1 and s2
 ...
 *  #VRNA_INPUT_NOPRINT_COMMENTS, #VRNA_INPUT_NOSKIP_COMMENTS,
 *#VRNA_INPUT_NOELIM_WS_SUFFIX
 *
 *  pass a collection of options as one value like this:
 *  \verbatim get_input_line(string, option_1 | option_2 | option_n)
 *\endverbatim
 *
 *  If the function recognizes the type of input, it will report it in the
 *return
 *  value. It also reports if a user defined 'quit' command (@-sign on 'stdin')
 *  was given. Possible return values are:\n
 *  #VRNA_INPUT_FASTA_HEADER, #VRNA_INPUT_ERROR, #VRNA_INPUT_MISC,
 *#VRNA_INPUT_QUIT
 *
 ...
 *
 *  This function may be used to obtain complete datasets from stdin. A dataset
is always
 *  defined to contain at least a sequence. If data on stdin starts with a fasta
header,
 *  i.e. a line like
 *  \verbatim >some header info \endverbatim
 *  then read_record() will assume that the sequence that follows the header may
span
 *  over several lines. To disable this behavior and to assign a single line to
the argument
 *  'sequence' one can pass VRNA_INPUT_NO_SPAN in the 'options' argument.
 *  If no fasta header is read in the beginning of a data block, a sequence must
not span over
 *  multiple lines!\n
 *  Unless the options #VRNA_INPUT_NOSKIP_COMMENTS or
#VRNA_INPUT_NOSKIP_BLANK_LINES are passed,
 *  a sequence may be interrupted by lines starting with a comment character or
empty lines.\n
 *  A sequence is regarded as completely read if it was either assumed to not
span over multiple
 *  lines, a secondary structure or structure constraint follows the sequence on
the next line
...
a new dataset are
 *  available through the line-array 'rest'. Here one can usually find the
structure constraint or
 *  other information belonging to the current dataset. Filling of 'rest' may be
prevented by
 *  passing #VRNA_INPUT_NO_REST to the options argument.\n
 *
 *  \note This function will exit any program with an error message if no
sequence could be read!
 *
 *  The main purpose of this function is to be able to easily parse blocks of
data from stdin
 *  in the header of a loop where all calculations for the appropriate data is
done inside the
 *  loop. The loop may be then left on certain return values, e.g.:
 *  \verbatim
char *id, *seq, **rest;
int  i;
while(!(read_record(&id, &seq, &rest, 0) & (VRNA_INPUT_ERROR |
VRNA_INPUT_QUIT))){
  if(id) printf("%s\n", id);
  printf("%s\n", seq);
  if(rest)
 ...
 *
 *  In the example above, the while loop will be terminated when read_record()
returns either an
 *  error or a user initiated quit request.\n
 *  As long as data is read from stdin, the id is printed if it is available for
the current block
 *  of data. The sequence will be printed in any case and if some more lines
belong to the current
 *  block of data each line will be printed as well.
 *
 *  \note Do not forget to free the memory occupied by header, sequence and
rest!
 *
 *  \param  header    A pointer which will be set such that it points to the
header of the record
 *  \param  sequence  A pointer which will be set such that it points to the
sequence of the record
 *  \param  rest      A pointer which will be set such that it points to an
array of lines which also belong to the record
 *  \param  options   Some options which may be passed to alter the behavior of
the function, use 0 for no options
 *  \return           A flag with information about what the function actually
did read
 */
 ...
 *
 * This function extracts a dot-bracket structure string from the 'rest' array
 *as
 * returned by read_record() and returns it. All occurences of comments within
 *the
 * 'lines' array will be skipped as long as they do not break the structure
 *string.
 ...
 * \param length  The assumed length of the dot-bracket string (passing a value
 *< 1 results in no length limit)
 * \param option  Some options which may be passed to alter the behavior of the
 *function, use 0 for no options
 * \return        The dot-bracket string read from lines or NULL
 */
char *extract_record_rest_structure(const char **lines, unsigned int length,
                                    unsigned int option);

/**
 *  \brief Pack secondary secondary structure, 5:1 compression using base 3
 *encoding
 *
...
/**
 *  \brief Compute the "base pair" distance between two secondary structures s1
 *and s2.
 *
 ...
 *
 *  There will also be a ruler (scale line) printed that helps orientation of
 ...
 *
 *  This function substitudes <i>T</i> and <i>t</i> with <i>U</i> and <i>u</i>,
 *respectively
 *
...
/**
 *  \brief Get an index mapper array (iindx) for accessing the energy matrices,
 *e.g. in partition function related functions.
 *
 *  Access of a position "(i,j)" is then accomplished by using \verbatim (i,j) ~
 *iindx[i]-j \endverbatim
 *  This function is necessary as most of the two-dimensional energy matrices
 *are actually one-dimensional arrays throughout
...
/**
 *  \brief Get an index mapper array (indx) for accessing the energy matrices,
 *e.g. in MFE related functions.
 *
 *  Access of a position "(i,j)" is then accomplished by using \verbatim (i,j) ~
 *indx[j]+i \endverbatim
 *  This function is necessary as most of the two-dimensional energy matrices
 *are actually one-dimensional arrays throughout
 *  the ViennaRNAPackage
 *
...
/**
 *  \brief Insert constraining pair types according to constraint structure
 *string
 *
 *  \see get_indx(), get_iindx()
 *
 *  \param constraint     The structure constraint string
 *  \param length         The actual length of the sequence (constraint may be
 *shorter)
 *  \param ptype          A pointer to the basepair type array
 *  \param min_loop_size  The minimal loop size (usually \ref TURN )
 *  \param idx_type       Define the access type for base pair type array (0 =
 *indx, 1 = iindx)
 */
void constrain_ptypes(const char *constraint, unsigned int length, char *ptype,
                      int *BP, int min_loop_size, unsigned int idx_type);
