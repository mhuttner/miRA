...
PRIVATE void initialize_Lfold(int length, int maxdist) {
  ...
  if (length < 1)
    nrerror("initialize_Lfold: argument must be greater 0");
  ...
  for (i = length - TURN - 1; i >= 1; i--) { /* i,j in [1..length] */
    for (j = i + TURN + 1; j <= length && j <= i + maxdist; j++) {
      ...
      if (type) { /* we have a pair */
        ...
        if (!no_close) {
          ...
          default:
            decomp += E_MLstem(tt, -1, -1, P);
            decomp =
                MIN2(decomp, DMLi2[j - 1 - (i + 2)] +
                                 E_MLstem(tt, -1, S1[i + 1], P) + P->MLbase);
            decomp = MIN2(decomp, DMLi2[j - 2 - (i + 2)] +
                                      E_MLstem(tt, S1[j - 1], S1[i + 1], P) +
                                      2 * P->MLbase);
            decomp =
                MIN2(decomp, DMLi1[j - 2 - (i + 1)] +
        ...
        if (dangles == 3) {
          ...
          for (k = i + 2 + TURN; k < j - 2 - TURN; k++) {
            ...
            if (type_2)
              decomp =
                  MIN2(decomp, c[i + 1][k - i - 1] + P->stack[type][type_2] +
                                   fML[k + 1][j - 1 - k - 1]);
            type_2 = ptype[k + 1][j - 1 - k - 1];
            type_2 = rtype[type_2];
            if (type_2)
              decomp = MIN2(decomp, c[k + 1][j - 1 - k - 1] +
                                        P->stack[type][type_2] +
                                        fML[i + 1][k - i - 1]);
          }
      ...
      case 2:
        ...
        new_fML = MIN2(new_fML, c[i][j - i] +
                                    E_MLstem(type, (i > 1) ? S1[i - 1] : -1,
                                             (j < length) ? S1[j + 1] : -1, P));
        break;
      /* normal dangles, aka dangles = 1 */
      default: /* i unpaired */
        ...
        tt = ptype[i + 1][j - i - 1];
        if (tt)
          new_fML = MIN2(new_fML, c[i + 1][j - i - 1] +
                                      E_MLstem(tt, S1[i], -1, P) + P->MLbase);
        /* i, j-1 */
        tt = ptype[i][j - 1 - i];
        if (tt)
          new_fML = MIN2(new_fML, c[i][j - 1 - i] + E_MLstem(tt, -1, S1[j], P) +
                                      P->MLbase);
        /* i+1,j-1 */
        tt = ptype[i + 1][j - 1 - i - 1];
        if (tt)
          new_fML =
              MIN2(new_fML, c[i + 1][j - 1 - i - 1] +
                                E_MLstem(tt, S1[i], S1[j], P) + 2 * P->MLbase);
        break;
      }
      ...
      if (dangles == 3) {
        ...
        for (decomp = INF, k = i + 1 + TURN; k <= j - 2 - TURN; k++) {
          ...
          type_2 = rtype[type_2];
          if (type && type_2)
            decomp = MIN2(decomp, c[i][k - i] + c[k + 1][j - k - 1] +
                                      P->stack[type][type_2]);
        }
...
#pragma omp threadprivate(do_backtrack, prev_i)
      ...
      case 0:
        for (j = i + TURN + 1; j < length && j <= i + maxdist; j++) {
          ...
          if (type)
            f3[i] = MIN2(f3[i],
                         f3[j + 1] + c[i][j - i] + E_ExtLoop(type, -1, -1, P));
        }
        if (length <= i + maxdist) {
          j = length;
      ...
      default:
        for (j = i + TURN + 1; j < length && j <= i + maxdist; j++) {
          ...
          if (type) {
            f3[i] = MIN2(f3[i],
                         f3[j + 1] + c[i][j - i] + E_ExtLoop(type, -1, -1, P));
            f3[i] =
                MIN2(f3[i], ((j + 2 <= length) ? f3[j + 2] : 0) + c[i][j - i] +
                                E_ExtLoop(type, -1, S1[j + 1], P));
          }
          type = ptype[i + 1][j - i - 1];
          if (type) {
            f3[i] = MIN2(f3[i], f3[j + 1] + c[i + 1][j - i - 1] +
                                    E_ExtLoop(type, S1[i], -1, P));
            f3[i] = MIN2(f3[i], ((j + 1 < length) ? f3[j + 2] : 0) +
                                    c[i + 1][j - i - 1] +
        ...
        if (length <= i + maxdist) {
          ...
          if (type)
            f3[i] = MIN2(f3[i],
                         c[i + 1][j - i - 1] + E_ExtLoop(type, S1[i], -1, P));
        }
        break;
      } /* switch(dangles)... */
      ...
      if (f3[i] < f3[i + 1])
        ...
        /*get pairpartner*/
        for (pairpartner = lind + TURN; pairpartner <= lind + maxdist;
             pairpartner++) {
            ...
            if (type) {
              cc = c[lind][pairpartner - lind] +
                   E_ExtLoop(type, (lind > 1) ? S1[lind - 1] : -1,
                             (pairpartner < length) ? S1[pairpartner + 1] : -1,
                             P);
              if (fij == cc + f3[pairpartner + 1])
                traced2 = 1;
            } else if (with_gquad) {
          ...
          default:
            if (type) {
              ...
              if (fij == cc + f3[pairpartner + 1]) {
                ...
                cc = c[lind][pairpartner - lind] +
                     E_ExtLoop(type, -1, S1[pairpartner + 1], P);
...
PRIVATE char *backtrack(const char *string, int start, int maxdist) {
    ...
    if (ml == 0) { /* backtrack in f3 */
      ...
      case 2:
        for (traced = 0, k = j; k > i + TURN; k--) {
          ...
          if (type)
            if (fij ==
                c[i][k - i] + E_ExtLoop(type, (i > 1) ? S1[i - 1] : -1,
                                        (k < length) ? S1[k + 1] : -1, P) +
                    f3[k + 1]) {
              traced = i;
              break;
      ...
      default:
        for (traced = 0, k = j; k > i + TURN; k--) {
          ...
          if (type) {
            if (fij ==
                c[i + 1][k - (i + 1)] + E_ExtLoop(type, S1[i], -1, P) +
                    f3[k + 1]) {
              traced = i + 1;
            }
            if (k < length) {
              if (fij ==
                  c[i + 1][k - (i + 1)] + E_ExtLoop(type, S1[i], S1[k + 1], P) +
                      f3[k + 2]) {
                traced = i + 1;
                jj = k + 2;
              }
          ...
          if (type) {
            ...
            if (k < length) {
              if (fij ==
                  c[i][k - i] + E_ExtLoop(type, -1, S1[k + 1], P) + f3[k + 2]) {
                traced = i;
                jj = k + 2;
              }
            }
      ...
      if (!traced)
        nrerror("backtrack failed in f3");
      ...
      case 2:
        tt = ptype[i][j - i];
        if (fij ==
            c[i][j - i] + E_MLstem(tt, (i > 1) ? S1[i - 1] : -1,
                                   (j < length) ? S1[j + 1] : -1, P)) {
          structure[i - start] = '(';
          structure[j - start] = ')';
          goto repeat1;
      ...
      default:
        ...
        tt = ptype[i + 1][j - (i + 1)];
        if (fij ==
            c[i + 1][j - (i + 1)] + E_MLstem(tt, S1[i], -1, P) + P->MLbase) {
          structure[++i - start] = '(';
          structure[j - start] = ')';
          goto repeat1;
        }
        ...
        tt = ptype[i + 1][j - 1 - (i + 1)];
        if (fij ==
            c[i + 1][j - 1 - (i + 1)] + E_MLstem(tt, S1[i], S1[j], P) +
                2 * P->MLbase) {
          structure[++i - start] = '(';
          structure[--j - start] = ')';
          goto repeat1;
  ...
  repeat1:
    ...
    if (no_close) {
      if (cij == FORBIDDEN)
        continue;
    } else if (cij == E_Hairpin(j - i - 1, type, S1[i + 1], S1[j - 1],
                                string + i - 1, P))
      continue;
    ...
    if (with_gquad) {
      ...
      */
      if (backtrack_GQuad_IntLoop_L(cij, i, j, type, S, ggg, maxdist, &p, &q,
                                    P)) {
        i = p;
        j = q;
        goto repeat_gquad;
    ...
    default:
      ...
      mm53 =
          P->MLclosing + E_MLstem(tt, S1[j - 1], S1[i + 1], P) + 2 * P->MLbase;
      for (k = i + 2 + TURN; k < j - 2 - TURN; k++) {
        if (cij == fML[i + 1][k - (i + 1)] + fML[k + 1][j - 1 - (k + 1)] + mm)
          break;
        else if (cij ==
                 fML[i + 2][k - (i + 2)] + fML[k + 1][j - 1 - (k + 1)] + mm3) {
          i1 = i + 2;
          break;
        } else if (cij ==
                   fML[i + 1][k - (i + 1)] + fML[k + 1][j - 2 - (k + 1)] +
                       mm5) {
          j1 = j - 2;
          break;
        } else if (cij ==
                   fML[i + 2][k - (i + 2)] + fML[k + 1][j - 2 - (k + 1)] +
                       mm53) {
          i1 = i + 2;
          j1 = j - 2;
          break;
        ...
        if (dangles == 3) {
          ...
          type_2 = rtype[type_2];
          if (type_2) {
            en = c[i + 1][k - (i + 1)] + P->stack[type][type_2] +
                 fML[k + 1][j - 1 - (k + 1)];
          ...
          type_2 = rtype[type_2];
          if (type_2) {
            en = c[k + 1][j - 1 - (k + 1)] + P->stack[type][type_2] +
                 fML[i + 1][k - (i + 1)];
            if (cij == en + 2 * P->MLintern[1] + P->MLclosing) {
              sector[s + 2].ml = 2;
              break;
